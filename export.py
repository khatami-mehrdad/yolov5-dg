...
def run(
    ...
    if any((saved_model, pb, tflite, edgetpu, tfjs)):  # TensorFlow formats
        ...
        f[5], keras_model = export_saved_model(model.cpu(),
                                           im,
                                           file,
                                           dynamic,
        ...
        if pb or tfjs:  # pb prerequisite to tfjs
            f[6], _ = export_pb(keras_model, file)
        if tflite or edgetpu:
            f[7], _ = export_tflite(keras_model, im, file, int8=int8 or edgetpu, data=data, nms=nms, agnostic_nms=agnostic_nms, max_int8_img_cnt=max_int8_img_cnt)
        if edgetpu:
